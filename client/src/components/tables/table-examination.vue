      ...
      <table class="w-full table-fixed">
        ...
        <tbody>
          <tr v-for="(data, index) in fetched" :key="index"  class="hover:bg-tosca group hover:text-gr bg-shade-gr text-white border border-gr w-full cursor-pointer transition-colors duration-300 ease-out text-[0.75rem]">
            ...
            <td class="px-4 py-2 truncate text-center align-middle">{{ new Date(data?.examinationDate).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' }) }}</td>
            <td class="px-4 py-2 truncate align-middle">
              <div v-if="data.grade !== 0" class="flex gap-8 justify-center items-center">
                <button @click.prevent="doOpenModal('createExamination', {id: data.id ,type : data.type, code: data.code, examinationDate: data.examinationDate, SubjectId: data.Subject.id })" class="flex gap-1 items-center justify-center hover:bg-shade-wh p-1 rounded-md ease-out transition-all duration-200">
                  <IkonPerbarui />
                  <div>Perbarui</div>
                </button>
