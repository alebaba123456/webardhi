<template>
    <div class="w-full h-full flex relative bg-pattern bg-fixed bg-repeat" ref="mainRef">
        ...
        <div class="font-extrabold text-tosca bg-gradient-to-r from-gr via-shade-gr to-transparent rounded-xl flex px-2 text-[1.5rem] mb-6 w-full">SOAL UJIAN</div>
            <FilterBar class="w-full mb-4" ref="filter" />
            <TableQuestion class="w-full my-2"/>
...
import TableQuestion from '@/components/tables/table-question.vue';
...
const { getQuestion, resetStates } = useStore;
const { size, category, keyword } = storeToRefs(useStore)
